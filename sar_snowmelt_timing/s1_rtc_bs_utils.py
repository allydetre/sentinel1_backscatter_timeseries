...
Original Author: Eric Gagliano (egagli@uw.edu)
Editor: Ally Detre (a.detre@colostate.edu)
Updated: 10/2024
"""
...
#                     end_time (str): end time of returned data 'YYYY-MM-DD'
#                     polarization (str): SAR polarization, use gamma0_vv
#                     resolution (int): spatial resolution of data (e.g., 20 for 20m resolution)
...
#     # Extract bounding box from bbox_gdf
#     bbox = tuple(bbox_gdf.total_bounds)
...
#     # Get the items returned from the search
#     items = search.item_collection()
#     print(f"Items found: {len(items)}")  # Debug: Print number of items found
    
#     # Check the first few items (for debugging)
#     if len(items) > 0:
#         print(f"First item: {items[0]}")  # Debug: Print first item in the collection
    
#     stack = stackstac.stack(items, bounds_latlon=bbox, epsg=4326, dtype='float', chunksize=512, resolution=resolution)
#     print(f"Stack bounds: {stack.rio.bounds()}")
#     print(f"Stack CRS: {stack.rio.crs}")
#     print(f"Stack resolution: {stack.rio.resolution()}")
#     print(f"Stack x coordinates: {stack.coords['x']}")
#     print(f"Stack y coordinates: {stack.coords['y']}")
    
#     # Perform CRS conversion and get bounding box
#     bounding_box_utm_gf = bbox_gdf.to_crs(stack.crs)
#     xmin, ymax, xmax, ymin = bounding_box_utm_gf.bounds.values[0]
...
#     "https://planetarycomputer.microsoft.com/api/stac/v1",
#     modifier=planetary_computer.sign_inplace,)
#     bbox = tuple(bbox_gdf.total_bounds) # I CHANGED THIS TO A TUPLE BC STACKSTAC HAVING ISSUES
...
#     items = search.item_collection()
#     stack = stackstac.stack(items, bounds_latlon=bbox, epsg=4326, dtype='float32',chunksize=512, resolution=resolution) #put resolution back in when fixed
#     # I CHANGED FLOAT32 TO FLOAT
#     bounding_box_utm_gf = bbox_gdf.to_crs(stack.crs)
#     xmin, ymax, xmax, ymin = bounding_box_utm_gf.bounds.values[0]
...
def get_s1_rtc_stac_pc(bbox_gdf,start_time='2014-01-01',end_time=datetime.today().strftime('%Y-%m-%d'),polarization='vv',resolution=20, epsg_code=4326):
    '''
    Returns a Sentinel-1 SAR backscatter xarray dataset using STAC data from Planetary computer over the given time and bounding box.
    ...
    search = catalog.search(collections=["sentinel-1-rtc"], bbox=bbox, datetime=f"{start_time}/{end_time}", limit=1000) #remove limit if needed
    items = search.item_collection()
    stack = stackstac.stack(items, bounds_latlon=bbox, epsg=epsg_code, dtype='float32',chunksize=512, resolution=resolution) #put resolution back in when fixed
    bounding_box_utm_gf = bbox_gdf.to_crs(stack.crs)
    xmin, ymax, xmax, ymin = bounding_box_utm_gf.bounds.values[0]
...
def get_s1_rtc_stac_pc_utm(bbox_gdf, start_time='2014-01-01', end_time=datetime.today().strftime('%Y-%m-%d'), 
                        polarization='vv', resolution=20, epsg_code=32613):
    '''
    Returns a Sentinel-1 SAR backscatter xarray dataset using STAC data from Planetary Computer 
    over the given time and bounding box for UTM coordinates.

    Parameters:
        bbox_gdf (geopandas GeoDataframe): geodataframe bounding box in UTM
        start_time (str): start time of returned data 'YYYY-MM-DD'
        end_time (str): end time of returned data 'YYYY-MM-DD'
        polarization (str): SAR polarization
        resolution (int): resolution of the output data
        epsg_code (int): EPSG code for the coordinate reference system of the bounding box
    ...
    # Get the UTM bounding box
    bbox = bbox_gdf.total_bounds  # Assuming this gives UTM coordinates
    search = catalog.search(collections=["sentinel-1-rtc"], bbox=bbox, datetime=f"{start_time}/{end_time}", limit=1000)
    items = search.item_collection()

    # Stack with UTM bounding box
    stack = stackstac.stack(items, bounds_latlon=None, epsg=epsg_code, dtype='float32', chunksize=512, resolution=resolution)

    # Create the scenes based on the UTM coordinates
    xmin, ymin, xmax, ymax = bbox  # UTM coordinates
    if polarization == 'all':
        scenes = stack.sel(x=slice(xmin, xmax), y=slice(ymin, ymax))
    else:
        scenes = stack.sel(band=polarization).sel(x=slice(xmin, xmax), y=slice(ymin, ymax))
...
def get_s1_rtc_stac_odc_pc(bbox_gdf,start_time='2019-01-01',end_time='2019-12-31',resolution=10,epsg=None, resampling=None):
    import odc.stac,odc
...
def get_s1_rtc_stac_odc_pc_vvvh(bbox_gdf,start_time='2019-01-01',end_time='2019-12-31',resolution=10,epsg=None):
    import odc.stac,odc
...
# Ally's modification to catch empty orbits
def get_runoff_onset(ts_ds, return_seperate_orbits_and_polarizations=False, num_acquisitions_during_melt_season=8):
    ...
    # Check if orbits is empty (handling numpy/xarray)
    if orbits.size == 0:
        print("Skipping year: No valid relative orbits found.")
        return None  # Skip processing if no valid orbits
    ...
    if not return_seperate_orbits_and_polarizations:  # Default: return median
